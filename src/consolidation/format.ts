...
/**
 * Compact currency formatting kept in {@link Money}/Decimal until the very last
 * step (AGENTS.md money rule: never float-divide currency for display).
 */
export function formatMoneyCompact(money: Money): string {
  ...
  return `${sign}${symbol}${compact.replace(/^-/, "")}`;
}
